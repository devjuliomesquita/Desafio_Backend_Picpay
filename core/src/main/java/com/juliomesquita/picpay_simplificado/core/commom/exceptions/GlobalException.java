...
public class GlobalException {
    ...
    @ExceptionHandler(NotificationErrorException.class)
    public ResponseEntity<CustomErrorResponse> handle(NotificationErrorException e) {
        Map<String, String> error = new HashMap<>();
        error.put("Notification Error", e.getMessage());
        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(new CustomErrorResponse(error));
    }
    ...
    public ResponseEntity<CustomErrorResponse> handle(HttpClientErrorException e) {
        Map<String, String> error = new HashMap<>();
        error.put("Transação não autorizada.", e.getMessage());
        return ResponseEntity
